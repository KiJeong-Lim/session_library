...
if [[ $* == *-help* ]]; then 
    ...
    do
        ...
        for w in "${workload[@]}"
        do
            mkdir -p "$OUT_DIR/$name/workload_$w"
            for session in {0..5} 
            do  
                mkdir -p "$OUT_DIR/$name/workload_$w/$session"
                for run in {1..3}
                do
                    mkdir -p "$OUT_DIR/$name/workload_$w/$session/run_$run"
                    for i in {1..10}
                    do
                        ...
                        tmux send-keys -t "$SES:0" C-c
                        tmux send-keys -t "$SES:1" C-c
                        tmux send-keys -t "$SES:2" C-c
